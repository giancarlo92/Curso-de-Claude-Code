...
- `.claude/skills/create-github-pull-request` — automatiza el flujo diario de abrir un PR: push de la rama actual, apertura del formulario de comparación en el navegador conectado del usuario (Claude in Chrome), redacción de título/descripción, asignación al propio usuario y clic en "Create pull request". A diferencia de otros flujos, este skill sí envía el formulario sin pedir confirmación adicional por cada clic, porque el usuario autorizó ese paso de forma explícita y recurrente.
